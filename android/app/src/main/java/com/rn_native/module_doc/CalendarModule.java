...
public class CalendarModule extends ReactContextBaseJavaModule {
   ...
   // Callback : native module cũng hỗ trợ 1 loại tham số nữa là callback để có thể
   // từ native kích hoạt chạy lại 1 hàm gì đó phía bên RN
   @ReactMethod
   public void createCalendarEvent_Callback(String name, String location, Callback callBack) { // Tham số 3: Hàm RN
                                                                                               // truyền sang
      Log.d(TAG, "createCalendar have_run_Callback_side_RN: " + name
            + " and location: " + location);
      Integer eventId = 100;
      callBack.invoke(eventId); // Chạy hàm bên RN truyền sang -- Chạy bên RN với tham số được truyền vào là
                                // eventId

      // WritableMap params = Arguments.createMap();
      // params.putString("eventProperty", "someValue");
      // sendEvent(getReactApplicationContext(), "EventReminder", params); //Tham số
      // 2: Tên Event -Tham số 3: Tham số gửi sang RN
   }

   // Có 1 lưu ý quan trọng đó là tại 1 thời điểm chỉ chạy được 1 callback , có
   // nghĩa là chỉ có thể chạy cái success hoặc fail không thể chạy cả 2
   // Có 2 cách để implement :

   // Có 2 cách để quyết đinh sẽ chạy hàm callback nào bên phía React native : 1
   // logic được viết bên RN , 2 logic được viết bên native , RN sẽ truyền 2
   // callback sang
   ...
   @ReactMethod
   public void createCalendarEvent_Callback_logicNative(String name, String location, Callback myFailureCallback,
   ...
   // Promise: Trả ra 1 promise để bên RN có thể async/ await để có thể lấy về kết
   // quả
   @ReactMethod
   ...
   // Để có thể kích hoạt 1 event mà bên RN đăng ký nhận event đó phát ra từ bên
   // native
